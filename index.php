<?php
include 'db.php';
// ── Fetch all active plans grouped by category ───────────────
$allPlans = [];
$planRes  = $conn->query("SELECT * FROM plans WHERE status='Active' ORDER BY FIELD(category,'Broadband','Mobile','DTH'), price+0 ASC");
while ($row = $planRes->fetch_assoc()) {
    $allPlans[$row['category']][] = $row;
}
$bbPlans  = $allPlans['Broadband'] ?? [];
$mobPlans = $allPlans['Mobile']    ?? [];
$dthPlans = $allPlans['DTH']       ?? [];
?>
            ...
            <section class="section">
                <div class="container">
                    ...
                    <div class="plans-container">
                        <?php
                        $heroPlans = array_slice($bbPlans, 0, 3);
                        if (empty($heroPlans)):
                        ?>
                        <p style="text-align:center;color:var(--NetServe-gray);">No plans available at the moment.</p>
                        <?php else: foreach ($heroPlans as $hp): ?>
                        <div class="card plan-card">
                            <h3 class="plan-name"><?= htmlspecialchars($hp['plan_name']) ?></h3>
                            <div class="plan-price">₹<?= htmlspecialchars($hp['price']) ?></div>
                            <div class="plan-duration"><?= htmlspecialchars($hp['validity'] ?: 'per month') ?></div>
                            <p style="margin: 20px 0; color: var(--NetServe-gray);"><?= htmlspecialchars($hp['description'] ?: '') ?></p>
            ...
            <section class="section">
                <div class="container">
                    ...
                    <div class="plans-grid">
                        <?php if (empty($bbPlans)): ?>
                        <p style="text-align:center;color:var(--NetServe-gray);padding:40px 0;">No broadband plans available. Please check back soon.</p>
                        <?php else: foreach ($bbPlans as $bIdx => $bp):
                            $isPopular = ($bIdx === 1); // 2nd plan = most popular
                        ?>
                        <div class="card<?= $isPopular ? ' plan-card popular' : '' ?>">
                            <?php if ($isPopular): ?><div class="popular-badge">Most Popular</div><?php endif; ?>
                            <div class="plan-header">
                                <h3 class="plan-name"><?= htmlspecialchars($bp['plan_name']) ?></h3>
                                <div class="plan-price">₹<?= htmlspecialchars($bp['price']) ?></div>
                                <div class="plan-duration"><?= htmlspecialchars($bp['validity'] ?: 'per month') ?></div>
                            </div>
                            <div class="plan-features">
                                <?php
                                $features = array_filter(array_map('trim', explode('|', $bp['description'] ?? '')));
                                $fIcons = ['fas fa-bolt','fas fa-infinity','fas fa-phone','fas fa-wifi','fas fa-tv','fas fa-star'];
                                foreach ($features as $fIdx => $feat): ?>
                                <div class="feature">
                                    <div class="feature-icon"><i class="<?= $fIcons[$fIdx % count($fIcons)] ?>"></i></div>
                                    <span><?= htmlspecialchars(trim($feat)) ?></span>
                                </div>
                                <?php endforeach; ?>
                            </div>
                            <div class="plan-footer">
                                <button class="btn" onclick="selectPlan('<?= addslashes(htmlspecialchars($bp['plan_name'])) ?>', <?= (int)$bp['price'] ?>)">Get This Plan</button>
                            </div>
                        </div>
                        <?php endforeach; endif; ?>
                    </div>
        ...
        <div id="mobile-page" class="page-content">
            <section class="section">
                <div class="container">
                    ...
                    <?php
                    // Split mobile plans into Prepaid and Postpaid by description keyword
                    $prepaidPlans  = array_filter($mobPlans, fn($p) => stripos($p['description'],'prepaid') !== false || stripos($p['plan_name'],'prepaid') !== false);
                    $postpaidPlans = array_filter($mobPlans, fn($p) => stripos($p['description'],'postpaid') !== false || stripos($p['plan_name'],'postpaid') !== false);
                    // Fallback: if no keyword match, split by position (first half prepaid)
                    if (empty($prepaidPlans) && empty($postpaidPlans)) {
                        $half = (int)ceil(count($mobPlans)/2);
                        $prepaidPlans  = array_slice($mobPlans, 0, $half);
                        $postpaidPlans = array_slice($mobPlans, $half);
                    }
                    ?>
                    <div id="prepaid-plans">
                        <table class="plans-table">
                            <thead><tr><th>Plan</th><th>Validity</th><th>Details</th><th>Price</th><th>Action</th></tr></thead>
                            <tbody>
                            <?php if (empty($prepaidPlans)): ?>
                            <tr><td colspan="5" style="text-align:center;padding:30px;color:#888;">No prepaid plans available.</td></tr>
                            <?php else: foreach ($prepaidPlans as $mp): ?>
                            <tr>
                                <td><?= htmlspecialchars($mp['plan_name']) ?></td>
                                <td><?= htmlspecialchars($mp['validity'] ?: '—') ?></td>
                                <td><?= htmlspecialchars($mp['description'] ?: '—') ?></td>
                                <td>₹<?= htmlspecialchars($mp['price']) ?></td>
                                <td><button class="btn" onclick="selectMobilePlan('<?= addslashes(htmlspecialchars($mp['plan_name'])) ?>', <?= (int)$mp['price'] ?>)">Buy Now</button></td>
                            </tr>
                            <?php endforeach; endif; ?>
                            </tbody>
                        </table>
                    ...
                    <div id="postpaid-plans" style="display: none;">
                        <table class="plans-table">
                            <thead><tr><th>Plan</th><th>Validity</th><th>Details</th><th>Price</th><th>Action</th></tr></thead>
                            <tbody>
                            <?php if (empty($postpaidPlans)): ?>
                            <tr><td colspan="5" style="text-align:center;padding:30px;color:#888;">No postpaid plans available.</td></tr>
                            <?php else: foreach ($postpaidPlans as $mp): ?>
                            <tr>
                                <td><?= htmlspecialchars($mp['plan_name']) ?></td>
                                <td><?= htmlspecialchars($mp['validity'] ?: '—') ?></td>
                                <td><?= htmlspecialchars($mp['description'] ?: '—') ?></td>
            ...
            <section class="section">
                <div class="container">
                    ...
                    <div class="channel-grid">
                        <?php
                        $dthIcons = ['fas fa-satellite-dish','fas fa-tv','fas fa-film','fas fa-star','fas fa-globe'];
                        if (empty($dthPlans)):
                        ?>
                        <p style="text-align:center;color:var(--NetServe-gray);padding:40px 0;">No DTH plans available. Please check back soon.</p>
                        <?php else: foreach ($dthPlans as $dIdx => $dp): ?>
                        <div class="card channel-card">
                            <div class="channel-icon">
                                <i class="<?= $dthIcons[$dIdx % count($dthIcons)] ?>"></i>
                            </div>
                            <h3 class="channel-name"><?= htmlspecialchars($dp['plan_name']) ?></h3>
                            <p style="color: var(--NetServe-gray); margin-bottom: 20px;"><?= htmlspecialchars($dp['description'] ?: '') ?></p>
                            <div class="plan-price">₹<?= htmlspecialchars($dp['price']) ?></div>
                            <div class="plan-duration"><?= htmlspecialchars($dp['validity'] ?: 'per month') ?></div>
                            <button class="btn" style="margin-top: 20px;" onclick="selectDTHPlan('<?= addslashes(htmlspecialchars($dp['plan_name'])) ?>', <?= (int)$dp['price'] ?>)">Subscribe</button>
                        </div>
                        <?php endforeach; endif; ?>
                    </div>
                </div>
...
<div class="modal-overlay" id="plan-modal">
    <div class="modal">
        ...
        <form id="plan-form" action="submit-form.php" method="POST" novalidate>
            ...
            <div class="form-group">
                ...
                <input type="tel" class="form-input" id="mobile" name="mobile"
                       placeholder="Enter your 10-digit mobile number" maxlength="10" autocomplete="tel">
                <small class="field-error" id="err-mobile"></small>
            </div>
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-input" id="address" name="address"
                       placeholder="Enter your complete address" autocomplete="street-address">
            ...
            <div class="form-group">
                ...
                <small class="field-error" id="err-plan"></small>
            </div>
    ...
    <script>
        ...
        function setupForms() {
            ...
            ['full-name','mobile','email','address'].forEach(function(id) {
                document.getElementById(id).addEventListener('input', function() {
                    var errId = 'err-' + (id === 'full-name' ? 'name' : id);
                    clearError(id, errId);
                });
            });
            ...
            document.getElementById('plan-form').addEventListener('submit', function(e) {
                var nameVal    = document.getElementById('full-name').value.trim();
                ...
                var planVal    = document.getElementById('plan-selected').value.trim();

                // Clear all previous errors first
                clearError('full-name', 'err-name');
                clearError('mobile',    'err-mobile');
                clearError('email',     'err-email');
                clearError('address',   'err-address');
                clearError('plan-selected', 'err-plan');

                var hasError = false;

                // Name
                if (nameVal === '') {
                    setError('full-name', 'err-name', 'Full name is required.'); hasError = true;
                } else if (!/^[a-zA-Z\s]+$/.test(nameVal)) {
                    setError('full-name', 'err-name', 'Name must contain letters only.'); hasError = true;
                } else if (nameVal.length < 3) {
                    setError('full-name', 'err-name', 'Name must be at least 3 characters.'); hasError = true;
                }

                // Mobile
                if (mobileVal === '') {
                    setError('mobile', 'err-mobile', 'Mobile number is required.'); hasError = true;
                } else if (!/^[6-9][0-9]{9}$/.test(mobileVal)) {
                    setError('mobile', 'err-mobile', 'Enter a valid 10-digit number starting with 6-9.'); hasError = true;
                }

                // Email
                if (emailVal === '') {
                    setError('email', 'err-email', 'Email address is required.'); hasError = true;
                } else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(emailVal)) {
                    setError('email', 'err-email', 'Please enter a valid email address.'); hasError = true;
                }

                // Address
                if (addressVal === '') {
                    setError('address', 'err-address', 'Address is required.'); hasError = true;
                } else if (addressVal.length < 10) {
                    setError('address', 'err-address', 'Please enter a complete address (at least 10 characters).'); hasError = true;
                }

                // Plan
                if (planVal === '') {
                    setError('plan-selected', 'err-plan', 'No plan selected. Please choose a plan first.'); hasError = true;
                }
                ...
                // Form submits normally to submit-form.php
            });
            // Close modal
